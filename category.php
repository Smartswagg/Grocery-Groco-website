...
// Validate category
$valid_categories = ['fruits', 'vegetables', 'fish', 'meat'];
$category_name = $_GET['category'] ?? '';
if (!in_array($category_name, $valid_categories)) {
    echo '<div style="padding:2rem;text-align:center;color:red;font-size:2rem;">Category not found. <a href="shop.php">Go back to shop</a></div>';
    exit;
}
...
if(isset($_POST['add_to_cart'])){
   ...
   if($check_cart_numbers->rowCount() > 0){
      ...
      // Insert into cart (this was missing)
...
<section class="products">
   ...
   <?php
      $category_name = $_GET['category'];
      $select_products = $conn->prepare("SELECT * FROM `products` WHERE category = ? AND status = 'approved'");
